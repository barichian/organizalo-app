...
import WhatsappLogo from "@/app/assets/services/whatsapp.png?url";
// components
...
const integrationDetails: { [key: string]: any } = {
  ...
  whatsapp: {
    logo: WhatsappLogo,
    description: "Receive task notifications and updates on WhatsApp.",
  },
};
    ...
    <>
      {integration && (
        <div className="flex items-center justify-between gap-2 border-b border-custom-border-100 bg-custom-background-100 px-4 py-6">
          ...
          {integration.integration_detail.provider === "whatsapp" && (
            <div className="text-sm text-custom-text-200">
              WhatsApp integration is active for this workspace. Use the command menu to sync specific tasks.
            </div>
          )}
        </div>
      )}
